...
.PARAMETER SaveLogToDevice
    If specified, logs are saved to C:\Logs\OneDriveInfo.log on the device.
...
param(
    ...
    # Custom field name for OneDrive sync client information
    [string]$OneDriveSyncClientFieldName = $(if ($env:oneDriveSyncClientFieldName) { $env:oneDriveSyncClientFieldName } else { "OneDriveStatusReport" })

    # Card customization options
    [string]$CardTitle = "OneDrive Config Details",  # Default Card title
    [string]$CardIcon = "fas fa-cloud",  # Default Card icon
...
begin {
    ...
    Write-Log "INFO" "Using custom field name: $OneDriveSyncClientFieldName"
}
...
process {
    try {
        ...
        $CombinedHTML | Ninja-Property-Set-Piped -Name $OneDriveSyncClientFieldName
    }
    catch {
